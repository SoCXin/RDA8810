#include "rda_sensor.h"
#include <linux/delay.h>

#ifdef RESERVE
#undef RESERVE
#endif
#define RESERVE	8
static struct sensor_reg exp_gc2145[][3] =
{
	{{0xfe,0x01,RESERVE,0},{0x13,0x50,RESERVE,0},{0xfe,0x00,RESERVE,0}},
	{{0xfe,0x01,RESERVE,0},{0x13,0x60,RESERVE,0},{0xfe,0x00,RESERVE,0}},
	{{0xfe,0x01,RESERVE,0},{0x13,0x64,RESERVE,0},{0xfe,0x00,RESERVE,0}},
	{{0xfe,0x01,RESERVE,0},{0x13,0x68,RESERVE,0},{0xfe,0x00,RESERVE,0}},//7b bob
	{{0xfe,0x01,RESERVE,0},{0x13,0x80,RESERVE,0},{0xfe,0x00,RESERVE,0}},
	{{0xfe,0x01,RESERVE,0},{0x13,0x90,RESERVE,0},{0xfe,0x00,RESERVE,0}},
	{{0xfe,0x01,RESERVE,0},{0x13,0xa0,RESERVE,0},{0xfe,0x00,RESERVE,0}},
};

static struct sensor_reg awb_gc2145[][4] =
{
	{{0x82,0xfc,RESERVE,0},{0xb3,0x61,RESERVE,0},{0xb4,0x40,RESERVE,0},{0xb5,0x61,RESERVE,0}},//OFF
	{{0x82,0xfe,RESERVE,0},{0xb3,0x61,RESERVE,0},{0xb4,0x40,RESERVE,0},{0xb5,0x61,RESERVE,0}},//AUTO
	{{0x82,0xfc,RESERVE,0},{0xb3,0x50,RESERVE,0},{0xb4,0x40,RESERVE,0},{0xb5,0xa8,RESERVE,0}},//INCANDESCENT
	{{0x82,0xfc,RESERVE,0},{0xb3,0x4e,RESERVE,0},{0xb4,0x40,RESERVE,0},{0xb5,0x72,RESERVE,0}},//FLUORESCENT
	{{0x82,0xfc,RESERVE,0},{0xb3,0xa0,RESERVE,0},{0xb4,0x45,RESERVE,0},{0xb5,0x40,RESERVE,0}},//TUNGSTEN
	{{0x82,0xfc,RESERVE,0},{0xb3,0x70,RESERVE,0},{0xb4,0x40,RESERVE,0},{0xb5,0x50,RESERVE,0}},//DAYLIGHT
	{{0x82,0xfc,RESERVE,0},{0xb3,0x58,RESERVE,0},{0xb4,0x40,RESERVE,0},{0xb5,0x50,RESERVE,0}},//CLOUD
};

// use this for 176x144 (QCIF) capture
static const struct sensor_reg qcif_gc2145[] =
{
	//need to be add
};

// use this for 160x120 (QQVGA) capture
static const struct sensor_reg qqvga_gc2145[] =
{
	//need to be add
};

// use this for 320x240 (QVGA) capture
static const struct sensor_reg qvga_gc2145[] =
{
	//need to be add
};

// use this for 640x480 (VGA) capture
static const struct sensor_reg vga_gc2145[] =
{
	//SENSORDB("GC2145_Sensor_SVGA", 0},

	{0xfe, 0x00, RESERVE,0},
	{0xfd, 0x01, RESERVE,0},
	{0xfa, 0x00, RESERVE,0},
	//// crop window
	{0xfe, 0x00, RESERVE,0},
	{0x90, 0x01, RESERVE,0},
	{0x91, 0x00, RESERVE,0},
	{0x92, 0x00, RESERVE,0},
	{0x93, 0x00, RESERVE,0},
	{0x94, 0x00, RESERVE,0},
	{0x95, 0x01, RESERVE,0},
	{0x96, 0xe0, RESERVE,0},
	{0x97, 0x02, RESERVE,0},
	{0x98, 0x80, RESERVE,0},
	{0x99, 0x55, RESERVE,0},
	{0x9a, 0x06, RESERVE,0},
	{0x9b, 0x01, RESERVE, 0},
	{0x9c, 0x34, RESERVE, 0},
	{0x9d, 0x00, RESERVE, 0},
	{0x9e, 0x00, RESERVE, 0},
	{0x9f, 0x01, RESERVE, 0},
	{0xa0, 0x34, RESERVE, 0},
	{0xa1, 0x00, RESERVE, 0},
	{0xa2, 0x00, RESERVE, 0},
	{0xfe, 0x00, RESERVE,0},
	{0xec, 0x02, RESERVE,0},
	{0xed, 0x02, RESERVE,0},
	{0xee, 0x30, RESERVE,0},
	{0xef, 0x48, RESERVE,0},
	{0xfe, 0x02, RESERVE,0},
	{0x9d, 0x08, RESERVE,0},
	{0xfe, 0x01, RESERVE,0},
	{0x74, 0x00, RESERVE,0},
	//// AEC
	{0xfe, 0x01, RESERVE,0},
	{0x01, 0x04, RESERVE,0},
	{0x02, 0x60, RESERVE,0},
	{0x03, 0x02, RESERVE,0},
	{0x04, 0x48, RESERVE,0},
	{0x05, 0x18, RESERVE,0},
	{0x06, 0x50, RESERVE,0},
	{0x07, 0x10, RESERVE,0},
	{0x08, 0x38, RESERVE,0},
	{0x0a, 0x80, RESERVE,0},
	{0x21, 0x04, RESERVE,0},
	{0xfe, 0x00, RESERVE,0},
	{0x20, 0x03, RESERVE,0},
	{0xfe, 0x00, RESERVE,0},

};

// use this for 1600x1200 (UXGA) capture
static const struct sensor_reg uxga_gc2145[] =
{
	{0xfe , 0x00, RESERVE,0},
	{0xfd , 0x00, RESERVE,0},
	{0xfa , 0x11, RESERVE,0},
	//// crop window
	{0xfe , 0x00, RESERVE,0},
	{0x90 , 0x01, RESERVE,0},
	{0x91 , 0x00, RESERVE,0},
	{0x92 , 0x00, RESERVE,0},
	{0x93 , 0x00, RESERVE,0},
	{0x94 , 0x00, RESERVE,0},
	{0x95 , 0x04, RESERVE,0},
	{0x96 , 0xb0, RESERVE,0},
	{0x97 , 0x06, RESERVE,0},
	{0x98 , 0x40, RESERVE,0},
	{0x99 , 0x11, RESERVE,0},
	{0x9a , 0x06, RESERVE,0},
	//// AWB
	{0xfe , 0x00, RESERVE,0},
	{0xec , 0x06, RESERVE,0},
	{0xed , 0x04, RESERVE,0},
	{0xee , 0x60, RESERVE,0},
	{0xef , 0x90, RESERVE,0},
	{0xfe , 0x01, RESERVE,0},
	{0x74 , 0x01, RESERVE,0},
	//// AEC
	{0xfe , 0x01, RESERVE,0},
	{0x01 , 0x04, RESERVE,0},
	{0x02 , 0xc0, RESERVE,0},
	{0x03 , 0x04, RESERVE,0},
	{0x04 , 0x90, RESERVE,0},
	{0x05 , 0x30, RESERVE,0},
	{0x06 , 0x90, RESERVE,0},
	{0x07 , 0x30, RESERVE,0},
	{0x08 , 0x80, RESERVE,0},
	{0x0a , 0x82, RESERVE,0},
	{0xfe , 0x01, RESERVE,0},
	{0x21 , 0x15, RESERVE,0},
	{0xfe , 0x00, RESERVE,0},
	{0x20 , 0x15, RESERVE,0},
	{0xfe , 0x00, RESERVE,0},
};

// use this for init sensor GC2145_CSI
static const struct sensor_reg init_gc2145[] =
{
	{0xfe , 0xf0, RESERVE,0},
	{0xfe , 0xf0, RESERVE,0},
	{0xfe , 0xf0, RESERVE,0},
	{0xfc , 0x06, RESERVE,0},
	{0xf6 , 0x00, RESERVE,0},
	{0xf7 , 0x1d, RESERVE,0},
	{0xf8 , 0x84, RESERVE,0},
	{0xfa , 0x00, RESERVE,0},
	{0xf9 , 0xfe, RESERVE,0},
	{0xf2 , 0x00, RESERVE,0},
	/////////////////////////////////////////////////
	//////////////////ISP reg//////////////////////
	////////////////////////////////////////////////////
	{0xfe , 0x00, RESERVE,0},
	{0x03 , 0x04, RESERVE,0},
	{0x04 , 0xe2, RESERVE,0},
	{0x09 , 0x00, RESERVE,0},
	{0x0a , 0x00, RESERVE,0},
	{0x0b , 0x00, RESERVE,0},
	{0x0c , 0x00, RESERVE,0},
	{0x0d , 0x04, RESERVE,0},
	{0x0e , 0xc0, RESERVE,0},
	{0x0f , 0x06, RESERVE,0},
	{0x10 , 0x52, RESERVE,0},
	{0x12 , 0x2e, RESERVE,0},
	{0x17 , 0x14, RESERVE,0}, //mirror
	{0x18 , 0x22, RESERVE,0},
	{0x19 , 0x0e, RESERVE,0},
	{0x1a , 0x01, RESERVE,0},
	{0x1b , 0x4b, RESERVE,0},
	{0x1c , 0x07, RESERVE,0},
	{0x1d , 0x10, RESERVE,0},
	{0x1e , 0x88, RESERVE,0},
	{0x1f , 0x78, RESERVE,0},
	{0x20 , 0x03, RESERVE,0},
	{0x21 , 0x40, RESERVE,0},
	{0x22 , 0xa0, RESERVE,0},
	{0x24 , 0x16, RESERVE,0},
	{0x25 , 0x01, RESERVE,0},
	{0x26 , 0x10, RESERVE,0},
	{0x2d , 0x60, RESERVE,0},
	{0x30 , 0x01, RESERVE,0},
	{0x31 , 0x90, RESERVE,0},
	{0x33 , 0x06, RESERVE,0},
	{0x34 , 0x01, RESERVE,0},
	/////////////////////////////////////////////////
	//////////////////ISP reg////////////////////
	/////////////////////////////////////////////////
	{0xfe , 0x00, RESERVE,0},
	{0x80 , 0x7f, RESERVE,0},
	{0x81 , 0x26, RESERVE,0},
	{0x82 , 0xfa, RESERVE,0},
	{0x83 , 0x00, RESERVE,0},
	{0x84 , 0x02, RESERVE,0}, //03
	{0x86 , 0x03, RESERVE,0},
	{0x88 , 0x03, RESERVE,0},
	{0x89 , 0x03, RESERVE,0},
	{0x85 , 0x08, RESERVE,0},
	{0x8a , 0x00, RESERVE,0},
	{0x8b , 0x00, RESERVE,0},
	{0xb0 , 0x55, RESERVE,0},
	{0xc3 , 0x00, RESERVE,0},
	{0xc4 , 0x80, RESERVE,0},
	{0xc5 , 0x90, RESERVE,0},
	{0xc6 , 0x3b, RESERVE,0},
	{0xc7 , 0x46, RESERVE,0},
	{0xec , 0x06, RESERVE,0},
	{0xed , 0x04, RESERVE,0},
	{0xee , 0x60, RESERVE,0},
	{0xef , 0x90, RESERVE,0},
	{0xb6 , 0x01, RESERVE,0},
	{0x90 , 0x01, RESERVE,0},
	{0x91 , 0x00, RESERVE,0},
	{0x92 , 0x00, RESERVE,0},
	{0x93 , 0x00, RESERVE,0},
	{0x94 , 0x00, RESERVE,0},
	{0x95 , 0x04, RESERVE,0},
	{0x96 , 0xb0, RESERVE,0},
	{0x97 , 0x06, RESERVE,0},
	{0x98 , 0x40, RESERVE,0},
	/////////////////////////////////////////
	/////////// BLK ////////////////////////
	/////////////////////////////////////////
	{0xfe , 0x00, RESERVE,0},
	{0x40 , 0x42, RESERVE,0},
	{0x41 , 0x00, RESERVE,0},
	{0x43 , 0x5b, RESERVE,0},
	{0x5e , 0x00, RESERVE,0},
	{0x5f , 0x00, RESERVE,0},
	{0x60 , 0x00, RESERVE,0},
	{0x61 , 0x00, RESERVE,0},
	{0x62 , 0x00, RESERVE,0},
	{0x63 , 0x00, RESERVE,0},
	{0x64 , 0x00, RESERVE,0},
	{0x65 , 0x00, RESERVE,0},
	{0x66 , 0x20, RESERVE,0},
	{0x67 , 0x20, RESERVE,0},
	{0x68 , 0x20, RESERVE,0},
	{0x69 , 0x20, RESERVE,0},
	{0x76 , 0x00, RESERVE,0},
	{0x6a , 0x08, RESERVE,0},
	{0x6b , 0x08, RESERVE,0},
	{0x6c , 0x08, RESERVE,0},
	{0x6d , 0x08, RESERVE,0},
	{0x6e , 0x08, RESERVE,0},
	{0x6f , 0x08, RESERVE,0},
	{0x70 , 0x08, RESERVE,0},
	{0x71 , 0x08, RESERVE,0},
	{0x76 , 0x00, RESERVE,0},
	{0x72 , 0xf0, RESERVE,0},
	{0x7e , 0x3c, RESERVE,0},
	{0x7f , 0x00, RESERVE,0},
	{0xfe , 0x02, RESERVE,0},
	{0x48 , 0x15, RESERVE,0},
	{0x49 , 0x00, RESERVE,0},
	{0x4b , 0x0b, RESERVE,0},
	{0xfe , 0x00, RESERVE,0},
	////////////////////////////////////////
	/////////// AEC ////////////////////////
	////////////////////////////////////////
	{0xfe , 0x01, RESERVE,0},
	{0x01 , 0x04, RESERVE,0},
	{0x02 , 0xc0, RESERVE,0},
	{0x03 , 0x04, RESERVE,0},
	{0x04 , 0x90, RESERVE,0},
	{0x05 , 0x30, RESERVE,0},
	{0x06 , 0x90, RESERVE,0},
	{0x07 , 0x30, RESERVE,0},
	{0x08 , 0x80, RESERVE,0},
	{0x09 , 0x00, RESERVE,0},
	{0x0a , 0x82, RESERVE,0},
	{0x0b , 0x11, RESERVE,0},
	{0x0c , 0x10, RESERVE,0},
	{0x11 , 0x10, RESERVE,0},
	{0x13 , 0x68, RESERVE,0},//7b->68 bob
	{0x17 , 0x00, RESERVE,0},
	{0x1c , 0x11, RESERVE,0},
	{0x1e , 0x61, RESERVE,0},
	{0x1f , 0x35, RESERVE,0},
	{0x20 , 0x40, RESERVE,0},
	{0x22 , 0x40, RESERVE,0},
	{0x23 , 0x20, RESERVE,0},
	{0xfe , 0x02, RESERVE,0},
	{0x0f , 0x04, RESERVE,0},
	{0xfe , 0x01, RESERVE,0},
	{0x12 , 0x30, RESERVE,0},//35
	{0x15 , 0xb0, RESERVE,0},
	{0x10 , 0x31, RESERVE,0},
	{0x3e , 0x28, RESERVE,0},
	{0x3f , 0xb0, RESERVE,0},
	{0x40 , 0x90, RESERVE,0},
	{0x41 , 0x0f, RESERVE,0},

	/////////////////////////////
	//////// INTPEE /////////////
	/////////////////////////////
	{0xfe , 0x02, RESERVE,0},
	{0x90 , 0x6c, RESERVE,0},
	{0x91 , 0x03, RESERVE,0},
	{0x92 , 0xcb, RESERVE,0},
	{0x94 , 0x33, RESERVE,0},
	{0x95 , 0x84, RESERVE,0},
	{0x97 , 0x65, RESERVE,0},// 54->65 bob
	{0xa2 , 0x11, RESERVE,0},
	{0xfe , 0x00, RESERVE,0},
	/////////////////////////////
	//////// DNDD///////////////
	/////////////////////////////
	{0xfe , 0x02, RESERVE,0},
	{0x80 , 0xc1, RESERVE,0},
	{0x81 , 0x08, RESERVE,0},
	{0x82 , 0x05, RESERVE,0},//05
	{0x83 , 0x08, RESERVE,0},//08
	{0x84 , 0x0a, RESERVE,0},
	{0x86 , 0xf0, RESERVE,0},
	{0x87 , 0x50, RESERVE,0},
	{0x88 , 0x15, RESERVE,0},
	{0x89 , 0xb0, RESERVE,0},
	{0x8a , 0x30, RESERVE,0},
	{0x8b , 0x10, RESERVE,0},
	/////////////////////////////////////////
	/////////// ASDE ////////////////////////
	/////////////////////////////////////////
	{0xfe , 0x01, RESERVE,0},
	{0x21 , 0x04, RESERVE,0},
	{0xfe , 0x02, RESERVE,0},
	{0xa3 , 0x50, RESERVE,0},
	{0xa4 , 0x20, RESERVE,0},
	{0xa5 , 0x40, RESERVE,0},
	{0xa6 , 0x80, RESERVE,0},
	{0xab , 0x40, RESERVE,0},
	{0xae , 0x0c, RESERVE,0},
	{0xb3 , 0x46, RESERVE,0},
	{0xb4 , 0x64, RESERVE,0},
	{0xb6 , 0x38, RESERVE,0},
	{0xb7 , 0x01, RESERVE,0},//01
	{0xb9 , 0x2b, RESERVE,0},//2b
	{0x3c , 0x04, RESERVE,0},//04
	{0x3d , 0x15, RESERVE,0},//15
	{0x4b , 0x06, RESERVE,0},//06
	{0x4c , 0x20, RESERVE,0},
	{0xfe , 0x00, RESERVE,0},
	/////////////////////////////////////////
	/////////// GAMMA   ////////////////////////
	/////////////////////////////////////////

	///////////////////gamma1////////////////////
#if 1
	{0xfe , 0x02, RESERVE,0},
	{0x10 , 0x09, RESERVE,0},
	{0x11 , 0x0d, RESERVE,0},
	{0x12 , 0x13, RESERVE,0},
	{0x13 , 0x19, RESERVE,0},
	{0x14 , 0x27, RESERVE,0},
	{0x15 , 0x37, RESERVE,0},
	{0x16 , 0x45, RESERVE,0},
	{0x17 , 0x53, RESERVE,0},
	{0x18 , 0x69, RESERVE,0},
	{0x19 , 0x7d, RESERVE,0},
	{0x1a , 0x8f, RESERVE,0},
	{0x1b , 0x9d, RESERVE,0},
	{0x1c , 0xa9, RESERVE,0},
	{0x1d , 0xbd, RESERVE,0},
	{0x1e , 0xcd, RESERVE,0},
	{0x1f , 0xd9, RESERVE,0},
	{0x20 , 0xe3, RESERVE,0},
	{0x21 , 0xea, RESERVE,0},
	{0x22 , 0xef, RESERVE,0},
	{0x23 , 0xf5, RESERVE,0},
	{0x24 , 0xf9, RESERVE,0},
	{0x25 , 0xff, RESERVE,0},
#else
	{0xfe , 0x02, 0},
	{0x10 , 0x0a, 0},
	{0x11 , 0x12, 0},
	{0x12 , 0x19, 0},
	{0x13 , 0x1f, 0},
	{0x14 , 0x2c, 0},
	{0x15 , 0x38, 0},
	{0x16 , 0x42, 0},
	{0x17 , 0x4e, 0},
	{0x18 , 0x63, 0},
	{0x19 , 0x76, 0},
	{0x1a , 0x87, 0},
	{0x1b , 0x96, 0},
	{0x1c , 0xa2, 0},
	{0x1d , 0xb8, 0},
	{0x1e , 0xcb, 0},
	{0x1f , 0xd8, 0},
	{0x20 , 0xe2, 0},
	{0x21 , 0xe9, 0},
	{0x22 , 0xf0, 0},
	{0x23 , 0xf8, 0},
	{0x24 , 0xfd, 0},
	{0x25 , 0xff, 0},
	{0xfe , 0x00, 0},
#endif
	{0xfe , 0x00, RESERVE,0},
	{0xc6 , 0x20, RESERVE,0},
	{0xc7 , 0x2b, RESERVE,0},
	///////////////////gamma2////////////////////
#if 1
	{0xfe , 0x02, RESERVE,0},
	{0x26 , 0x0f, RESERVE,0},
	{0x27 , 0x14, RESERVE,0},
	{0x28 , 0x19, RESERVE,0},
	{0x29 , 0x1e, RESERVE,0},
	{0x2a , 0x27, RESERVE,0},
	{0x2b , 0x33, RESERVE,0},
	{0x2c , 0x3b, RESERVE,0},
	{0x2d , 0x45, RESERVE,0},
	{0x2e , 0x59, RESERVE,0},
	{0x2f , 0x69, RESERVE,0},
	{0x30 , 0x7c, RESERVE,0},
	{0x31 , 0x89, RESERVE,0},
	{0x32 , 0x98, RESERVE,0},
	{0x33 , 0xae, RESERVE,0},
	{0x34 , 0xc0, RESERVE,0},
	{0x35 , 0xcf, RESERVE,0},
	{0x36 , 0xda, RESERVE,0},
	{0x37 , 0xe2, RESERVE,0},
	{0x38 , 0xe9, RESERVE,0},
	{0x39 , 0xf3, RESERVE,0},
	{0x3a , 0xf9, RESERVE,0},
	{0x3b , 0xff, RESERVE,0},
#else
	////Gamma outdoor
	{0xfe , 0x02, 0},
	{0x26 , 0x17, 0},
	{0x27 , 0x18, 0},
	{0x28 , 0x1c, 0},
	{0x29 , 0x20, 0},
	{0x2a , 0x28, 0},
	{0x2b , 0x34, 0},
	{0x2c , 0x40, 0},
	{0x2d , 0x49, 0},
	{0x2e , 0x5b, 0},
	{0x2f , 0x6d, 0},
	{0x30 , 0x7d, 0},
	{0x31 , 0x89, 0},
	{0x32 , 0x97, 0},
	{0x33 , 0xac, 0},
	{0x34 , 0xc0, 0},
	{0x35 , 0xcf, 0},
	{0x36 , 0xda, 0},
	{0x37 , 0xe5, 0},
	{0x38 , 0xec, 0},
	{0x39 , 0xf8, 0},
	{0x3a , 0xfd, 0},
	{0x3b , 0xff, 0},
#endif
	///////////////////////////////////////////////
	///////////YCP ///////////////////////
	///////////////////////////////////////////////
	{0xfe , 0x02, RESERVE,0},
	{0xd1 , 0x32, RESERVE,0},//32->2d
	{0xd2 , 0x32, RESERVE,0},//32->2d bob
	{0xd3 , 0x40, RESERVE,0},
	{0xd6 , 0xf0, RESERVE,0},
	{0xd7 , 0x10, RESERVE,0},
	{0xd8 , 0xda, RESERVE,0},
	{0xdd , 0x14, RESERVE,0},
	{0xde , 0x86, RESERVE,0},
	{0xed , 0x80, RESERVE,0},//80
	{0xee , 0x00, RESERVE,0},//00
	{0xef , 0x3f, RESERVE,0},
	{0xd8 , 0xd8, RESERVE,0},
	///////////////////abs/////////////////
	{0xfe , 0x01, RESERVE,0},
	{0x9f , 0x40, RESERVE,0},
	/////////////////////////////////////////////
	//////////////////////// LSC ///////////////
	//////////////////////////////////////////
	{0xfe , 0x01, RESERVE,0},
	{0xc2 , 0x14, RESERVE,0},
	{0xc3 , 0x0d, RESERVE,0},
	{0xc4 , 0x0c, RESERVE,0},
	{0xc8 , 0x15, RESERVE,0},
	{0xc9 , 0x0d, RESERVE,0},
	{0xca , 0x0a, RESERVE,0},
	{0xbc , 0x24, RESERVE,0},
	{0xbd , 0x10, RESERVE,0},
	{0xbe , 0x0b, RESERVE,0},
	{0xb6 , 0x25, RESERVE,0},
	{0xb7 , 0x16, RESERVE,0},
	{0xb8 , 0x15, RESERVE,0},
	{0xc5 , 0x00, RESERVE,0},
	{0xc6 , 0x00, RESERVE,0},
	{0xc7 , 0x00, RESERVE,0},
	{0xcb , 0x00, RESERVE,0},
	{0xcc , 0x00, RESERVE,0},
	{0xcd , 0x00, RESERVE,0},
	{0xbf , 0x07, RESERVE,0},
	{0xc0 , 0x00, RESERVE,0},
	{0xc1 , 0x00, RESERVE,0},
	{0xb9 , 0x00, RESERVE,0},
	{0xba , 0x00, RESERVE,0},
	{0xbb , 0x00, RESERVE,0},
	{0xaa , 0x01, RESERVE,0},
	{0xab , 0x01, RESERVE,0},
	{0xac , 0x00, RESERVE,0},
	{0xad , 0x05, RESERVE,0},
	{0xae , 0x06, RESERVE,0},
	{0xaf , 0x0e, RESERVE,0},
	{0xb0 , 0x0b, RESERVE,0},
	{0xb1 , 0x07, RESERVE,0},
	{0xb2 , 0x06, RESERVE,0},
	{0xb3 , 0x17, RESERVE,0},
	{0xb4 , 0x0e, RESERVE,0},
	{0xb5 , 0x0e, RESERVE,0},
	{0xd0 , 0x09, RESERVE,0},
	{0xd1 , 0x00, RESERVE,0},
	{0xd2 , 0x00, RESERVE,0},
	{0xd6 , 0x08, RESERVE,0},
	{0xd7 , 0x00, RESERVE,0},
	{0xd8 , 0x00, RESERVE,0},
	{0xd9 , 0x00, RESERVE,0},
	{0xda , 0x00, RESERVE,0},
	{0xdb , 0x00, RESERVE,0},
	{0xd3 , 0x0a, RESERVE,0},
	{0xd4 , 0x00, RESERVE,0},
	{0xd5 , 0x00, RESERVE,0},
	{0xa4 , 0x00, RESERVE,0},
	{0xa5 , 0x00, RESERVE,0},
	{0xa6 , 0x77, RESERVE,0},
	{0xa7 , 0x77, RESERVE,0},
	{0xa8 , 0x77, RESERVE,0},
	{0xa9 , 0x77, RESERVE,0},
	{0xa1 , 0x80, RESERVE,0},
	{0xa2 , 0x80, RESERVE,0},

	{0xfe , 0x01, RESERVE,0},
	{0xdf , 0x0d, RESERVE,0},
	{0xdc , 0x25, RESERVE,0},
	{0xdd , 0x30, RESERVE,0},
	{0xe0 , 0x77, RESERVE,0},
	{0xe1 , 0x80, RESERVE,0},
	{0xe2 , 0x77, RESERVE,0},
	{0xe3 , 0x90, RESERVE,0},
	{0xe6 , 0x90, RESERVE,0},
	{0xe7 , 0xa0, RESERVE,0},
	{0xe8 , 0x90, RESERVE,0},
	{0xe9 , 0xa0, RESERVE,0},
	{0xfe , 0x00, RESERVE,0},
	///////////////////////////////////////////////
	/////////// AWB////////////////////////
	///////////////////////////////////////////////
	{0xfe , 0x01, RESERVE,0},
	{0x4f , 0x00, RESERVE,0},
	{0x4f , 0x00, RESERVE,0},
	{0x4b , 0x01, RESERVE,0},
	{0x4f , 0x00, RESERVE,0},

	{0x4c , 0x01, RESERVE,0}, // D75
	{0x4d , 0x71, RESERVE,0},
	{0x4e , 0x01, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x91, RESERVE,0},
	{0x4e , 0x01, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x70, RESERVE,0},
	{0x4e , 0x01, RESERVE,0},
	{0x4c , 0x01, RESERVE,0}, // D65
	{0x4d , 0x90, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xb0, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x8f, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x6f, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xaf, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xd0, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xf0, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xcf, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xef, RESERVE,0},
	{0x4e , 0x02, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},//D50
	{0x4d , 0x6e, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x8e, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xae, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xce, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x4d, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x6d, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x8d, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xad, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xcd, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x4c, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x6c, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x8c, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xac, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xcc, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xcb, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x4b, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x6b, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0x8b, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xab, RESERVE,0},
	{0x4e , 0x03, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},//CWF
	{0x4d , 0x8a, RESERVE,0},
	{0x4e , 0x04, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xaa, RESERVE,0},
	{0x4e , 0x04, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xca, RESERVE,0},
	{0x4e , 0x04, RESERVE,0},
	{0x4c , 0x01, RESERVE,0},
	{0x4d , 0xca, RESERVE,0},
	{0x4e , 0x04, RESERVE, 0},
	{0x4c , 0x01, RESERVE, 0},
	{0x4d , 0xc9, RESERVE, 0},
	{0x4e , 0x04, RESERVE, 0},
	{0x4c , 0x01, RESERVE, 0},
	{0x4d , 0x8a, RESERVE, 0},
	{0x4e , 0x04, RESERVE, 0},
	{0x4c , 0x01, RESERVE, 0},
	{0x4d , 0x89, RESERVE, 0},
	{0x4e , 0x04, RESERVE, 0},
	{0x4c , 0x01, RESERVE, 0},
	{0x4d , 0xa9, RESERVE, 0},
	{0x4e , 0x04, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},//tl84
	{0x4d , 0x0b, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x0a, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	{0x4c , 0x01, RESERVE, 0},
	{0x4d , 0xeb, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	{0x4c , 0x01, RESERVE, 0},
	{0x4d , 0xea, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x09, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x29, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x2a, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x4a, RESERVE, 0},
	{0x4e , 0x05, RESERVE, 0},
	//{0x4c , 0x02, RESERVE, 0}, //A
	//{0x4d , 0x6a, RESERVE, 0},
	//{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x8a, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x49, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x69, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x89, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xa9, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x48, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x68, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0x69, RESERVE, 0},
	{0x4e , 0x06, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},//H
	{0x4d , 0xca, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xc9, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xe9, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x03, RESERVE, 0},
	{0x4d , 0x09, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xc8, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xe8, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xa7, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xc7, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x02, RESERVE, 0},
	{0x4d , 0xe7, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},
	{0x4c , 0x03, RESERVE, 0},
	{0x4d , 0x07, RESERVE, 0},
	{0x4e , 0x07, RESERVE, 0},

	{0x4f , 0x01, RESERVE, 0},
	{0x50 , 0x80, RESERVE, 0},
	{0x51 , 0xa8, RESERVE, 0},
	{0x52 , 0x47, RESERVE, 0},
	{0x53 , 0x38, RESERVE, 0},
	{0x54 , 0xc7, RESERVE, 0},
	{0x56 , 0x0e, RESERVE, 0},
	{0x58 , 0x08, RESERVE, 0},
	{0x5b , 0x00, RESERVE, 0},
	{0x5c , 0x74, RESERVE, 0},
	{0x5d , 0x8b, RESERVE, 0},
	{0x61 , 0xdb, RESERVE, 0},
	{0x62 , 0xb8, RESERVE, 0},
	{0x63 , 0x86, RESERVE, 0},
	{0x64 , 0xc0, RESERVE, 0},
	{0x65 , 0x04, RESERVE, 0},
	{0x67 , 0xa8, RESERVE, 0},
	{0x68 , 0xb0, RESERVE, 0},
	{0x69 , 0x00, RESERVE, 0},
	{0x6a , 0xa8, RESERVE, 0},
	{0x6b , 0xb0, RESERVE, 0},
	{0x6c , 0xaf, RESERVE, 0},
	{0x6d , 0x8b, RESERVE, 0},
	{0x6e , 0x50, RESERVE, 0},
	{0x6f , 0x18, RESERVE, 0},
	{0x73 , 0xf0, RESERVE, 0},
	{0x70 , 0x0d, RESERVE, 0},
	{0x71 , 0x60, RESERVE, 0},
	{0x72 , 0x80, RESERVE, 0},
	{0x74 , 0x01, RESERVE, 0},
	{0x75 , 0x01, RESERVE, 0},
	{0x7f , 0x0c, RESERVE, 0},
	{0x76 , 0x70, RESERVE, 0},
	{0x77 , 0x58, RESERVE, 0},
	{0x78 , 0xa0, RESERVE, 0},
	{0x79 , 0x5e, RESERVE, 0},
	{0x7a , 0x54, RESERVE, 0},
	{0x7b , 0x58, RESERVE, 0},
	{0xfe , 0x00, RESERVE, 0},
	//////////////////////////////////////////
	///////////CC////////////////////////
	//////////////////////////////////////////
	{0xfe , 0x02, RESERVE, 0},
	{0xc0 , 0x01, RESERVE, 0},
	{0xc1 , 0x44, RESERVE, 0},
	{0xc2 , 0xfd, RESERVE, 0},
	{0xc3 , 0x04, RESERVE, 0},
	{0xc4 , 0xF0, RESERVE, 0},
	{0xc5 , 0x48, RESERVE, 0},
	{0xc6 , 0xfd, RESERVE, 0},
	{0xc7 , 0x46, RESERVE, 0},
	{0xc8 , 0xfd, RESERVE, 0},
	{0xc9 , 0x02, RESERVE, 0},
	{0xca , 0xe0, RESERVE, 0},
	{0xcb , 0x45, RESERVE, 0},
	{0xcc , 0xec, RESERVE, 0},
	{0xcd , 0x48, RESERVE, 0},
	{0xce , 0xf0, RESERVE, 0},
	{0xcf , 0xf0, RESERVE, 0},
	{0xe3 , 0x0c, RESERVE, 0},
	{0xe4 , 0x4b, RESERVE, 0},
	{0xe5 , 0xe0, RESERVE, 0},
	//////////////////////////////////////////
	///////////ABS ////////////////////
	//////////////////////////////////////////
	{0xfe , 0x01, RESERVE, 0},
	{0x9f , 0x40, RESERVE, 0},
	{0xfe , 0x00, RESERVE, 0},
	//////////////////////////////////////
	///////////  OUTPUT   ////////////////
	//////////////////////////////////////
	{0xfe, 0x00, RESERVE, 0},
	{0xf2, 0x0f, RESERVE, 0},
	///////////////dark sun////////////////////
	{0xfe , 0x02, RESERVE, 0},
	{0x40 , 0xbf, RESERVE, 0},
	{0x46 , 0xcf, RESERVE, 0},
	{0xfe , 0x00, RESERVE, 0},

	//////////////frame rate 50Hz/////////
	/*{0xfe , 0x00, RESERVE, 0},
	{0x05 , 0x01, RESERVE, 0},  //156
	{0x06 , 0x56, RESERVE, 0},
	{0x07 , 0x00, RESERVE, 0},  //32
	{0x08 , 0x32, RESERVE, 0},
	{0xfe , 0x01, RESERVE, 0},
	{0x25 , 0x00, RESERVE, 0},  //fa
	{0x26 , 0xfa, RESERVE, 0},
	{0x27 , 0x07, RESERVE, 0},
	{0x28 , 0xd0, RESERVE, 0}, //20fps     4e2
	{0x29 , 0x09, RESERVE, 0},
	{0x2a , 0xc4, RESERVE, 0}, //14fps  6d6
	{0x2b , 0x0b, RESERVE, 0},
	{0x2c , 0xb8, RESERVE, 0}, //12fps    7d0
	{0x2d , 0x0b, RESERVE, 0},
	{0x2e , 0xb8, RESERVE, 0}, //8fps    bb8
	{0xfe , 0x00, RESERVE, 0},*/

	{0xfe , 0x00, RESERVE, 0},
	{0x05 , 0x01, RESERVE, 0},  //156
	{0x06 , 0x56, RESERVE, 0},
	{0x07 , 0x00, RESERVE, 0},  //32
	{0x08 , 0x32, RESERVE, 0},
	{0xfe , 0x01, RESERVE, 0},
	{0x25 , 0x00, RESERVE, 0},  //fa
	{0x26 , 0xfa, RESERVE, 0},
	{0x27 , 0x04, RESERVE, 0},
	{0x28 , 0xe2, RESERVE, 0}, //20fps     4e2
	{0x29 , 0x06, RESERVE, 0},
	{0x2a , 0xd6, RESERVE, 0}, //14fps  6d6
	{0x2b , 0x07, RESERVE, 0},
	{0x2c , 0xd0, RESERVE, 0}, //12fps    7d0
	{0x2d , 0x0b, RESERVE, 0},
	{0x2e , 0xb8, RESERVE, 0}, //8fps    bb8
	{0xfe , 0x00, RESERVE, 0},
};

static const struct sensor_reg_list gc2145_init =
{
	.size = ARRAY_ROW(init_gc2145),
	.val = init_gc2145
};
static const struct sensor_reg_list gc2145_uxga = {
	.size = ARRAY_ROW(uxga_gc2145),
	.val = uxga_gc2145
};
static const struct sensor_reg_list gc2145_vga = {
	.size = ARRAY_ROW(vga_gc2145),
	.val = vga_gc2145
};
static const struct sensor_reg_list gc2145_qvga = {
	.size = ARRAY_ROW(qvga_gc2145),
	.val = qvga_gc2145
};
static const struct sensor_reg_list gc2145_qcif = {
	.size = ARRAY_ROW(qcif_gc2145),
	.val = qcif_gc2145
};
static const struct sensor_reg_list gc2145_qqvga = {
	.size = ARRAY_ROW(qqvga_gc2145),
	.val = qqvga_gc2145
};
static const struct sensor_win_size gc2145_win_size[] = {
	WIN_SIZE("UXGA", W_UXGA, H_UXGA, &gc2145_uxga),
	WIN_SIZE("VGA", W_VGA, H_VGA, &gc2145_vga),
	WIN_SIZE("QVGA", W_QVGA, H_QVGA, &gc2145_qvga),
	WIN_SIZE("QCIF", W_QCIF, H_QCIF, &gc2145_qcif),
	WIN_SIZE("QQVGA", W_QQVGA, H_QQVGA, &gc2145_qqvga),
};

static const struct sensor_win_cfg gc2145_win_cfg = {
	.num = ARRAY_ROW(gc2145_win_size),
	.win_size = gc2145_win_size
};

static const struct sensor_csi_cfg gc2145_csi_cfg = {
	.csi_en = false,
	.d_term_en = 0,
	.c_term_en = 0,
	.dhs_settle = 0,
	.chs_settle = 0,
};

static struct sensor_info gc2145_info = {
	.name		= "gc2145",
	.chip_id	= 0x2145,
	.mclk		= 26,
	.i2c_addr	= 0x3C,
	.exp_def	= 0,
	.awb_def	= 1,
	.rst_act_h	= false,
	.pdn_act_h	= true,
	.init		= &gc2145_init,
	.win_cfg	= &gc2145_win_cfg,
	.csi_cfg	= &gc2145_csi_cfg
};

extern void sensor_power_down(bool high, bool acth, int id);
extern void sensor_reset(bool rst, bool acth);
extern void sensor_clock(bool out, int mclk);
extern void sensor_read(const u16 addr, u8 *data, u8 bits);
extern void sensor_write(const u16 addr, const u8 data, u8 bits);
extern void sensor_write_group(struct sensor_reg* reg, u32 size);

static u32 gc2145_power(int id, int mclk, bool rst_h, bool pdn_h)
{
	/* set state to power off */
	sensor_power_down(true, pdn_h, 0);
	mdelay(1);
	sensor_power_down(true, pdn_h, 1);
	mdelay(1);
	sensor_reset(true, rst_h);
	mdelay(1);
	sensor_clock(false, mclk);
	mdelay(5);

	/* power on sequence */
	sensor_clock(true, mclk);
	mdelay(1);
	sensor_power_down(false, pdn_h, id);
	mdelay(1);
	sensor_reset(false, rst_h);
	mdelay(10);

	return 0;
}

static u32 gc2145_get_chipid(void)
{
	u16 chip_id = 0;
	u8 tmp;

	sensor_read(0xf0, &tmp, RESERVE);
	chip_id = (tmp << 8) & 0xff00;
	sensor_read(0xf1, &tmp, RESERVE);
	chip_id |= (tmp & 0xff);
	printk("%s   chipid = 0x%4x\n",__func__,chip_id);
	return chip_id;
}

static u32 gc2145_get_lum(void)
{
	u8 val = 0;
	u32 ret = 0;

	sensor_write(0xfe, 0x01, RESERVE);
	sensor_read(0x14, &val, RESERVE);
	sensor_write(0xfe, 0x00, RESERVE);

	if (val < 0x50)
		ret = 1;

	return ret;
}

#define GC2145_FLIP_BASE	0x17
#define GC2145_H_FLIP_RESERVE	0
#define GC2145_V_FLIP_RESERVE	1
static void gc2145_set_flip(int hv, int flip)
{
	u8 tmp = 0;

	sensor_read(GC2145_FLIP_BASE, &tmp, RESERVE);

	if (hv) {
		if (flip)
			tmp |= (0x1 << GC2145_V_FLIP_RESERVE);
		else
			tmp &= ~(0x1 << GC2145_V_FLIP_RESERVE);
	}
	else {
		if (flip)
			tmp |= (0x1 << GC2145_H_FLIP_RESERVE);
		else
			tmp &= ~(0x1 << GC2145_H_FLIP_RESERVE);
	}

	sensor_write(GC2145_FLIP_BASE, tmp, RESERVE);
}

#define GC2145_EXP_ROW		ARRAY_ROW(exp_gc2145)
#define GC2145_EXP_COL		ARRAY_COL(exp_gc2145)
static void gc2145_set_exp(int exp)
{
	int key = exp + (GC2145_EXP_ROW / 2);
	if ((key < 0) || (key > (GC2145_EXP_ROW - 1)))
		return;

	sensor_write_group(exp_gc2145[key], GC2145_EXP_COL);
}

#define GC2145_AWB_ROW		ARRAY_ROW(awb_gc2145)
#define GC2145_AWB_COL		ARRAY_COL(awb_gc2145)
static void gc2145_set_awb(int awb)
{
	if ((awb < 0) || (awb > (GC2145_AWB_ROW - 1)))
		return;

	sensor_write_group(awb_gc2145[awb], GC2145_AWB_COL);
}

static struct sensor_ops gc2145_ops = {
	.power		= gc2145_power,
	.get_chipid	= gc2145_get_chipid,
	.get_lum	= gc2145_get_lum,
	.set_flip	= gc2145_set_flip,
	.set_exp	= gc2145_set_exp,
	.set_awb	= gc2145_set_awb,
	.start		= NULL,
	.stop		= NULL
};

struct sensor_dev gc2145_dev = {
	.info	= &gc2145_info,
	.ops	= &gc2145_ops,
};
