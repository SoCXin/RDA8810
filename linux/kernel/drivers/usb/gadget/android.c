		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (size >= sizeof(config->common->inquiry_string))
		return -EINVAL;
	strncpy(config->common->inquiry_string, buf,
...
static ssize_t cdrom_state_setcdr_show(struct device *dev,
                                struct device_attribute *attr, char *buf)
{
        struct android_usb_function *f = dev_get_drvdata(dev);
        struct mass_storage_function_config *config = f->config;
        return sprintf(buf, "%d\n", config->common->luns[0].cdrom);
}

static int fsg_nluns;
static ssize_t cdrom_state_setcdr_store(struct device *dev,
                struct device_attribute *attr, const char *buf, size_t size)
{
        struct android_usb_function *f = dev_get_drvdata(dev);
        struct mass_storage_function_config *config = f->config;
	...
	if(!fsg_nluns)
		...
		config->common->nluns = fsg_nluns;
        }
        return size;
}

static DEVICE_ATTR(setcdr, S_IRUGO | S_IWUSR,
					cdrom_state_setcdr_show,
					cdrom_state_setcdr_store);
