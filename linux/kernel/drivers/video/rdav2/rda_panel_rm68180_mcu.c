...
static int rm68180_mcu_reset_gpio(void)
{
	printk("%s\n",__func__);
...
static int rm68180_mcu_machid_sub(void)
{
	u16 data[6] = {0};
	u32 cmd = 0xc500;
...
static bool rm68180_mcu_match_id(void)
{
	u32 pwl_old, pwh_old;
	struct lcd_panel_info *lcd = (void *)&(rm68180_mcu_info.lcd);
	int ret = 0;

	rda_lcdc_pre_enable_lcd(lcd,1);
	rm68180_mcu_reset_gpio();
...
static int rm68180_mcu_open(void)
{
	...
	LCD_MCU_CMD(0xBC02); LCD_MCU_DATA(0x10);

	LCD_MCU_CMD(0xBD00); LCD_MCU_DATA(0x00);
	LCD_MCU_CMD(0xBD01); LCD_MCU_DATA(0x90);
	LCD_MCU_CMD(0xBD02); LCD_MCU_DATA(0x10);
	...
	LCD_MCU_CMD(0xF600); LCD_MCU_DATA(0xc0);//60

	LCD_MCU_CMD(0x3500); LCD_MCU_DATA(0x01);

	LCD_MCU_CMD(0x4400); LCD_MCU_DATA(0x00);
	LCD_MCU_CMD(0x4401); LCD_MCU_DATA(0x20);
...
static int rm68180_mcu_set_active_win(struct lcd_img_rect *r)
{
#if 0
	LCD_MCU_CMD(0x2a00);
...
static int rm68180_mcu_display_on(void)
{
	return 0;
}

static int rm68180_mcu_display_off(void)
{
	return 0;
}
...
static struct rda_lcd_info rm68180_mcu_info = {
	...
	.ops = {
		.s_reset_gpio = rm68180_mcu_reset_gpio,
		.s_open = rm68180_mcu_open,
		.s_match_id = rm68180_mcu_match_id,
		...
		.s_display_on = rm68180_mcu_display_on,
		.s_display_off = rm68180_mcu_display_off
	},
	.lcd = {
